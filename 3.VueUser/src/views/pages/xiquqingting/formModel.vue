<template>
	<div class="app-contain" :style='{"padding":"0px 18% 120px","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"url(http://clfile.zggen.cn/20231115/0bf98b23edb64e0eb248ded477134aac.jpg) no-repeat center bottom / 100% auto","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">戏曲介绍</div>
					<div  class="info_text" >{{detail.xiqujieshao}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">戏曲文件</div>
					<audio class="info_audio" :src="$config.url + detail.songfile" controls="controls">
						您的浏览器不支持音频播放
					</audio>
				</div>
				<div class="btn_view">
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('xiquqingting','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('xiquqingting','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('xiquqingting','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
	...
	const collectClick = (type) => {
		if (type == 1 && !collectType.value) {
			let params = {
				name: title.value,
				picture: bannerList.value[0],
				inteltype: detail.value.songname,
				refid: detail.value.id,
				type: type,
				tablename: tableName,
				userid: context?.$toolUtil.storageGet('userid')
...
<style lang="scss" scoped>
	...
	.detail_view{
		...
		.thumbs_view {
			...
			.zan:hover {
				transform: rotate(0deg) scale(1.09) skew(0deg, 0deg) translate3d(0px, -3px, 0px);
			}
			.zan:active {
				transform: scale(1);
			}
		...
		.info_view {
			...
			.btn_view {
				...
				// 悬浮
				.edit_btn:hover {
					transform: scale(1) translate3d(3px, 0px, 0px);
				}
				// 删除-按钮
				.del_btn {
	...
	.tabs_view {
		...
		.my_comment_view {
			...
			.comment_btn {
				...
				// 悬浮
				.add_btn:hover {
					transform: rotate(0deg) scale(1) skew(0deg, 0deg) translate3d(3px, 0, 0px);
				}
				// 重置按钮
				.reset_btn {
					border-radius: 0;
