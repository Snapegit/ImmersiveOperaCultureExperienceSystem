	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 18% 120px","margin":"0 auto 20px","overflow":"hidden","borderRadius":"4px","background":"url(http://clfile.zggen.cn/20231115/0bf98b23edb64e0eb248ded477134aac.jpg) no-repeat center bottom / 100% auto","width":"100%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('xiquqingting','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
		<div class="list_bottom">
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" @mouseenter="mouseenter(index)" @mouseleave="mouseleave">
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/xiquqingtingAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('xiquqingtingDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
		...
		for (let x in list.value) {
			if (changeAudio(list.value[x].id)) {
				arr.push({
                    ...
                    pic: context?.$config.url + list.value[x].fengmian.split(',')[0]
				})
